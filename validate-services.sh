...
for SERVICE in "${SERVICES[@]}"; do
    ...
    if [ ! -d "$SERVICE_DIR" ]; then
        ...
        echo "✅ Service folder exists: $SERVICE_DIR"
    fi
    ...
    if [ ! -f "$DOCKERFILE" ]; then
        ...
        echo "✅ Dockerfile exists in: $SERVICE_DIR"
    fi
done

if [ $ERRORS -eq 0 ]; then
    echo "🎉 All services and Dockerfiles are present!"
else
    echo "⚠️  Found $ERRORS issue(s). Fix them before pushing to CodeBuild."
    exit 1
fi
